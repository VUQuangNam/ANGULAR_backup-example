...
@Component({
    selector: 'dunnio-item-tailor-partial-update',
...
export class ItemTailorDetailComponent implements OnChanges, OnInit {
    ...
    async ngOnInit() {
        let respone;
        respone = await this.categoryService.list({ skip: 0, limit: 100 } as any);
        this.categories = respone.code ? [] : respone.data;
        respone = await this.serviceInventory.list(
            {
                skip: 0,
                limit: 20,
    ...
    async ngOnChanges() {
        try {
            ...
            if (this.model) {
                ...
                this.bodyNotesub = this.bodyNote.filter(x => includes(x.categories, this.model.category_id));

                // model metrics
                this.metrics = await this.designConfig.metrics;

                this.metrics = this.metrics.map(x => {
                    const check = includes(Object.keys(this.model.metrics), x.id);
                    if (check) {
                        if (x.input.total === 1) {
                            return {
                                key: x.id,
                                ...
                                genders: x.genders,
                                value: this.model.metrics[x.id]
                            };
                        }
                        if (x.input.total === 2) {
                            const key = this.model.metrics[x.id].split('/');
                            return {
                                key: x.id,
                                key1: key[0] || 0,
                                key2: key[1] || 0,
                ...
                this.designExtras = this.designExtras.map((x) => {
                    ...
                    const valueTmps = {
                        ...
                        values: x.values.map((v) => {
                            /** check lv2 */
                            const checkLV2 = checkLV1
                                ? this.model.design_extras[checkLV1].find((keylv2: { id: any; }) => keylv2.id === v.id)
                                : false;
                            ...
                            return {
                                ...
                                values: v.values.map((sv) => {
                                    let checkLV3 = false;
                                    const value = checkLV1
                                        ? this.model.design_extras[checkLV1].find((f: { id: any; }) => f.id === v.id)
                                        : false;
                                    if (value) {
    ...
    onSearchFabric = (keyword: string) => {
        ...
        this.keypress = setTimeout(
            async () => {
                try {
                    if (!keyword) return;
                    const respone = await this.serviceInventory.list(
                        {
                            skip: 0,
                            limit: 20,
                            keyword: keyword,
    ...
    onChangeGender = (genderId: string) => {
        ...
        this.onLoadDesign();
        /** find total item to set name and id */
        const totalItem = this.data.products.filter(x =>
            x.type === 'tailor' &&
            x.category_id === this.model.category_id
        );
        this.model.id = this.data.id + 'M' + (totalItem.length + 1);
        this.model.name = 'May đo ' + this.model.category_id;
    ...
    onUpdateItem = async () => {
        ...
        this.model.metricsSub = {};
        this.metricsSub.map(x => {
            if (x.value) {
                this.model.metricsSub[x.key] = x.value;
            }
        });
        ...
        /** create item */
        const respone = await this.service.update(this.itemId, this.model);
        if (respone.code) {
