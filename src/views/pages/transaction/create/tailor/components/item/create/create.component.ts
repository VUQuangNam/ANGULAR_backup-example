...
import { ProductService, CategoryService, DesignService, InventoryService } from 'src/common/services';
import { DesignConfig } from 'src/config';

@Component({
    selector: 'dunnio-item-tailor-partial',
...
export class ItemTailorPartialComponent implements OnInit, OnChanges {
    constructor(
    ...
    onCreateItem = async () => {
        try {
            ...
            return ($('#dunnio_create_item') as any).modal('hide');
        } catch (ex) {
            console.log(ex);
        }
    }
