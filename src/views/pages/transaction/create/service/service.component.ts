...
import { ProductService, OrderService, StoreService, VoucherService } from 'src/common/services';
...
export class OrderCreateWarrantyComponent implements OnInit {
    constructor(
    ...
    generateCart = async () => {
        try {
            ...
            const defaultData = {
                ...
                receive_process: {
                    date: null,
                    id: this.model.receive_process.id
                },
                note: {
                    system: '',
                    customer: ''
    ...
    onConfirmCartInfo = async () => {
        ...
        try {
            ...
            this.model.total_point = this.model.total_price * 15 / 100;
            ...
            this.model.products.forEach(x => {
                x.total_before_discount = x.price;
                delete x.status;
            });
    ...
    onSelectItem = async (target: any = {}) => {
        try {
            ...
            let respone;
            respone = await this.productSerive.detail(target.item.id.toLowerCase());
    ...
    onOpenUpdateItem = (event) => {
        ...
        ($('#dunnio_detail_item_warranties') as any).modal('show');
    }
